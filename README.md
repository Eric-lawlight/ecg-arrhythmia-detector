...
* **Random Forest classifier** with `class\_weight="balanced"` for real-world imbalanced data
...
└── src/
    ...
    │   └── mitbih\_loader.py  # MIT-BIH real data loader (wfdb)
    ├── features/
...
wfdb-dl mitdb --records 100 101 102 --output-dir ./mitbih\_data

python train.py --use-real-data --data-dir ./mitbih\_data
```
...
!\[Beat Classes](output/beat\_classes.png)
!\[Confusion Matrix](output/confusion\_matrix.png)
...
`fitbit\_ecg\_analysis.ipynb` 을 열면 실제 Fitbit Charge 5 ECG 데이터로
전체 파이프라인을 실행할 수 있습니다.



\# NOTE (2026-04): Fitbit Web API → Google Health API 마이그레이션 진행 중

\# (September 2026 전환 예정) — 실제 연동은 Google Health API 안정화 후 업데이트 예정
...
```bash
# 1. Fitbit 개발자 토큰 발급: Google Health API + Google OAuth 2.0
# 2. 노트북 실행
jupyter notebook fitbit\_ecg\_analysis.ipynb
# 3. ACCESS\_TOKEN 셀에 토큰 입력 후 전체 실행
# ※ 토큰 없이도 Demo Mode로 전체 파이프라인 확인 가능합니다
```

!\[Fitbit ECG](fitbit\_ecg\_waveforms.png)
